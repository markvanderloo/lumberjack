...
for what part of the script. This can be done by adding two lines of code.
\begin{Sinput}
  data(women) 
  ...
  dump_log()
\end{Sinput}
Second, we run our script using \code{lumberjack::run}.
<<>>=
library(lumberjack)
out <- run("process.R")
@
All variables created during \code{run} are stored in \code{out}.
<<>>=
head(out$women, 3)
@
...
\begin{enumerate}
  ...
  \item Close the logger and dump the logging information.
  \item Run the whole file using \code{lumberjack::run}.
\end{enumerate}
...
We have already seen the \code{run()} implementation of the \pkg{lumberjack} 
...
called. All loggers in \pkg{lumberjack} write output to a \code{csv} file with
default location \code{dataset\_logger.csv}. If \code{run} is used to to
execute an R file, then the log is written in the same directory as where the R
file resides.
...
@
The logging data consists of a step number, a timestamp, the expression that
transformed the data, the record key, the variable, the old and the new value.
