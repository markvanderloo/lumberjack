...
2. `$dump()` This function writes the current logging info somewhere.  Often
    this will be a file, but it really can be any place where R can send data.

Furthermore, a logger _must_ have a slot called `$label`. The label is set by
`start_log` and is used to keep track of logging streams when multiple datasets
are tracked with instances of the same logger type. `start_log` will try to
create a label if none is provided. If it fails to create a lable, it will
